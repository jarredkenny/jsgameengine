...
class Camera {
   ...
   */
  tileIsVisible(tile){
    return this.bodyIsVisible(tile);
  }

  /**
   * Body Is Visible
   * Checks if the provided body is within camera bounds.
   * @param Body body
   * @return Boolean isVisible
   */
  bodyIsVisible(body){
    return (
      body.position.x + body.size.x * this.zoom >= this.position.x - body.size.x &&
      body.position.x <= this.position.x + this.size.x * this.zoom &&
      body.position.y + body.size.y * this.zoom >= this.position.y  - body.size.y &&
      body.position.y <= this.position.y + this.size.y * this.zoom
    );
  }
